...
\node at (-27*\x,-2*\x) [] {\footnotesize{$130=\textcolor{acolor3}{3}+127$}};
...
\node at (-27*\x,-2*\x) [] {\footnotesize{$133=\textcolor{acolor3}{6}+127$}};
...
将十进制小数转换为二进制的方法：\textcolor{acolor3}{将小数乘2，取整数部分。}
\begin{figure}
\centering
...
\lstinputlisting[language=c,numbers=left,frame=single]{Code/float1.c}
\end{frame}
...
\begin{frame}[fragile]\ft{练习}
\lstinputlisting[language=c,numbers=left,frame=single]{Code/float2.c}
\end{frame}
...
\begin{frame}[fragile]\ft{练习}
\lstinputlisting[language=c,numbers=left,frame=single]{Code/float3.c}
\end{frame}
...
\begin{frame}[fragile]\ft{打印浮点型数据}
\lstinputlisting[language=c,numbers=left,frame=single]{Code/showf_pt.c}
\end{frame}
...
\lstinputlisting[language=c,numbers=left,frame=single]{Code/float_overflow.c}
\end{frame}
%
\begin{frame}[fragile]\ft{浮点型数据的上溢和下溢}\begin{lstlisting}[backgroundcolor=\color{red!10}]
...
\lstinputlisting[language=c,numbers=left,frame=single]{Code/float_err.c}
\end{frame}
%
\begin{frame}[fragile]\ft{浮点型数据舍入误差}\begin{lstlisting}[backgroundcolor=\color{red!10}]
$ gcc float_err.c
$ ./a.out
